...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      style,
      SUM(albums_track_count.price) / SUM(albums_track_count.count)
        AS price_per_track
    FROM
      styles
    JOIN (
      ...
      GROUP BY
        a.asin
      ) AS albums_track_count
        ON
        styles.album = albums_track_count.asin
      GROUP BY
        styles.style
      ORDER BY
        SUM(albums_track_count.price) / SUM(albums_track_count.count) DESC
      LIMIT 5
  SQL
end
